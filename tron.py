...
class Brain:
    ...
    def mutate(self):
        newBrain = Brain(layer_structure=self.layer_structure.copy(), coefs=self.mutateCoefs(),
                         intercepts=self.mutateIntercepts())
    ...
    def calculateOutput(self, input, g="identity"):
        ...
        for k in range(len(reduced_layer_structure)):
            ...
            result = np.matmul(np.transpose(self.coefs[k]), previousLayer) + np.transpose(
                np.array([self.intercepts[k]]))
...
def createBoxes():
    ...
    for j in range(GRIDSIZE[0]):
        for i in range(GRIDSIZE[1]):
            boxes.append(Box(x1=int(Xoffset + i * boxSide), y1=int(Yoffset + j * boxSide - boxSide / 2),
                             x2=int(Xoffset + (i + 1) * boxSide), y2=int(Yoffset + j * boxSide + boxSide / 2),
                             color=GRAY, index=index))
            index += 1
    return boxes
...
def displayNetwork(screen, layer_structure, coefs):
    ...
    inputs = ["distDown", "distUp", "distLeft", "distRight", "distDownRight", "distUpRight", "distDownLeft",
              "distUpLeft", "self X", "self Y", "enemy X", "enemy Y"]
...
while exit == False:
    ...
    for i in range(len(brains1)):
        ...
        while done == False:
            ...
            input1 = np.array([[distDown1] + [distUp1] + [distLeft1] + [distRight1] + [distDownRight1] + [
                distUpRight1] + [distDownLeft1] + [distUpLeft1] + [x1] + [y1] + [x2] + [y2]])
            input2 = np.array([[distDown2] + [distUp2] + [distLeft2] + [distRight2] + [distDownRight2] + [
                distUpRight2] + [distDownLeft2] + [distUpLeft2] + [x2] + [y2] + [x1] + [y1]])
            ...
            if P1Direction == LEFT:
                ...
                if boxes[RCtoNum([P1Position[0], P1Position[1] - 1])].free == False:
                    if boxes[RCtoNum([P1Position[0], P1Position[
                                                         1] - 1])].color == BLACK:  # Checks to see if the crash was with the border wall
